...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const verseContract = new ethers.Contract(addresses.VERSE_ADDRESS, VerseTokenContract, signer);
    const sverseContract = new ethers.Contract(addresses.SVERSE_ADDRESS, sVerseTokenContract, signer);
