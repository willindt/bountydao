...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const sverseContract = new ethers.Contract(addresses.SVERSE_ADDRESS, wsVerseTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await sverseContract.approve(addresses.WSVERSE_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wsverseAllowance = await sverseContract.allowance(address, addresses.WSVERSE_ADDRESS);
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wsverseContract = new ethers.Contract(addresses.WSVERSE_ADDRESS, wsVerseTokenContract, signer);
